...
lazy val sbtgen = (project in file("sbtgen"))
  ...
  .settings(
    ...
    libraryDependencies in ThisBuild += "org.scalatest" %% "scalatest" % "3.1.3" % "test",
    scalacOptions ++= Seq(
...
lazy val `sbt-izumi` = (project in file("sbt/sbt-izumi"))
  .settings(
    ...
    libraryDependencies ++= Seq(
      ...
      ("org.xerial.sbt" % "sbt-sonatype" % "3.9.4").extra(SbtVersionKey -> (sbtBinaryVersion in pluginCrossBuild).value, ScalaVersionKey -> (scalaBinaryVersion in update).value).withCrossVersion(Disabled()),
