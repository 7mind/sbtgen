...
lazy val `sbt-izumi` = (project in file("sbt/sbt-izumi"))
  .settings(
    ...
    libraryDependencies ++= Seq(
      ...
      ("org.scoverage" % "sbt-scoverage" % "1.9.2").extra(SbtVersionKey -> (pluginCrossBuild / sbtBinaryVersion).value, ScalaVersionKey -> (update / scalaBinaryVersion).value).withCrossVersion(Disabled()),
      ...
      ("org.xerial.sbt" % "sbt-sonatype" % "3.9.10").extra(SbtVersionKey -> (pluginCrossBuild / sbtBinaryVersion).value, ScalaVersionKey -> (update / scalaBinaryVersion).value).withCrossVersion(Disabled()),
